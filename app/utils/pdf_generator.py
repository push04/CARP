...
This version handles missing system dependencies gracefully
"""
import os
import tempfile
...
def generate_pdf_report(tenders, client_name="CARP BIOTECH PRIVATE LIMITED"):
    ...
    This version handles missing WeasyPrint dependencies gracefully
    """
    try:
        # First try to import WeasyPrint
        try:
            from weasyprint import HTML, CSS
            from weasyprint.text.fonts import FontConfiguration
            weasyprint_available = True
        except ImportError:
            logger.warning("WeasyPrint not available. Falling back to HTML generation.")
        ...
        if not weasyprint_available:
            # If WeasyPrint isn't available, just return the HTML content
            logger.info("Returning HTML content instead of PDF")
            return html_content
        ...
        # If PDF generation fails, return the HTML content as fallback
        logger.info("Falling back to HTML content")
        return html_content
...
def generate_html_report(tenders, client_name):
    ...
    <head>
        ...
        <style>
            ...
            .client-name { font-size: 24px; font-weight: bold; color: #2c3e50; margin-bottom: 5px; }
            .report-title { font-size: 18px; color: #7f8c8d; margin-bottom: 5px; }
            .report-date { font-size: 14px; color: #95a5a6; }
            .tender-block { border: 1px solid #ddd; border-radius: 8px; padding: 15px; margin-bottom: 20px; background-color: #f9f9f9; }
            .tender-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #eee; }
            .tender-title { font-size: 16px; font-weight: bold; color: #2c3e50; flex-grow: 1; }
            .tender-id { font-size: 12px; color: #7f8c8d; margin-left: 10px; }
            .tender-details { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-bottom: 10px; }
            .detail-item { margin-bottom: 5px; }
            .detail-label { font-weight: bold; color: #34495e; font-size: 12px; text-transform: uppercase; }
            .detail-value { font-size: 13px; color: #555; }
            .tender-description { font-size: 13px; color: #555; line-height: 1.5; margin-bottom: 10px; }
            .attachments { font-size: 12px; color: #2980b9; margin-bottom: 10px; }
            .checkbox-container { display: flex; align-items: center; margin-top: 10px; padding-top: 10px; border-top: 1px solid #eee; }
            .checkbox { width: 18px; height: 18px; margin-right: 10px; transform: scale(1.2); }
            .checkbox-label { font-size: 13px; color: #7f8c8d; }
            .footer { margin-top: 30px; text-align: center; font-size: 12px; color: #7f8c8d; border-top: 1px solid #eee; padding-top: 10px; }
        </style>
    </head>
    <body>
        <div class="header">
    ...
    for i, tender in enumerate(tenders, 1):
        ...
        if tender.attachments:
            try:
                ...
                if attachment_list:
                    attachments = f"<div class='attachments'><strong>Attachments:</strong><br>" + "<br>".join([
        ...
        <div class="tender-block">
            ...
            <div class="tender-details">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value">{"₹{:,.2f} {}".format(tender.tender_value, tender.currency or "INR") if tender.tender_value else "N/A"}</div>
                </div>
            </div>
