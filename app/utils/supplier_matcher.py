...
class SupplierMatcher:
    ...
    def __init__(self):
        try:
            ...
            logger.warning("AIAnalyzer unavailable in SupplierMatcher")
    ...
    def find_supplier_matches(self, tender: Tender, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            for supplier, score in scored_suppliers[:limit]:
                ...
                ai_analysis = {"match_percentage": round(score * 100, 2), "relevance_reasons": [], "estimated_price_range": {"min": 0, "max": 0}}
                if self.ai_analyzer:
                    try:
                        ...
                        logger.warning(f"AI analysis failed for supplier {supplier.name}: {e}")
    ...
    def _calculate_supplier_score(self, tender: Tender, supplier: Supplier, tender_keywords: List[str]) -> float:
        ...
        """
        score = 0.0
        supplier_products_text = (supplier.products_services or '').lower()
        
        # Fuzzy matching on supplier name vs tender title/description
        if supplier.name and tender.title:
        ...
        if supplier_products_text:
            tender_text = f"{tender.title or ''} {tender.description or ''}".lower()
            product_match = fuzz.partial_ratio(supplier_products_text, tender_text)
        ...
        if supplier_products_text:
            ...
            for keyword in tender_keywords:
                if keyword in supplier_products_text:
                    keyword_matches += 1
